  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in slide-in-from-left-4 duration-500">
      ...
      <div className="flex items-start gap-4 p-6 bg-gray-50/50 rounded-3xl border border-gray-100">
          ...
          <label htmlFor="waiver" className="text-sm text-gray-500 leading-relaxed font-medium">
            <div className="mb-1">
              我确认已满 18 周岁，已阅读并同意 <button type="button" onClick={() => setShowWaiverModal(true)} className="text-cny-red font-bold hover:underline decoration-2">上述所有条款（包含责任豁免、食品安全及肖像授权）</button>。我代表本人及本次报名中的所有未成年家属签署本协议。
            </div>
            <div className="text-xs text-gray-400 font-normal">
              I certify that I am at least 18 years old and have read and agree to <button type="button" onClick={() => setShowWaiverModal(true)} className="hover:text-cny-red hover:underline">all terms above, including the Liability Waiver, Food Safety Warning, and Media Release</button>. I sign this agreement on behalf of myself and all minors included in this registration.
            </div>
          </label>
      </div>
